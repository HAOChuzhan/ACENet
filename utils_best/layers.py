...
class MatrixVectorScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        if mask is not None:
            attn = attn.masked_fill(mask, -np.inf) # 将index为1的位置替换为-np.inf
...
class MultiheadAttPoolLayer(nn.Module):
    ...
    def forward(self, q, k, mask=None):
        """
        q: tensor of shape (b, d_q_original)    这里是sente的编码结果
        k: tensor of shape (b, l, d_k_original) 这里是gnn的所有node的输出结果
        mask: tensor of shape (b, l) (optional, default None) 这里是kg-Node type==3 为true
        returns: tensor of shape (b, n*d_v)
        """
        n_head, d_k, d_v = self.n_head, self.d_k, self.d_v # d_k=d_v=d_k_original//n_head
...
class SelfAttLayer(nn.Module):
    ...
    def forward(self, q, k, mask=None):
        """
        q: tensor of shape (b, d_q_original)    这里是sente的编码结果
        k: tensor of shape (b, d_k_original) 这里是gnn的所有node的输出结果
        mask: tensor of shape (b, l) (optional, default None) 这里是kg-Node type==3 为true
        returns: tensor of shape (b, n*d_v)
        """
        ...
        # print("qs_size:", qs.size())
        # print("vs_size:", vs.size())
        # print("ks_size:", ks.size())
        ...
        # print("qs2_size:", qs.size())
        # print("vs2_size:", vs.size())
        # print("ks2_size:", ks.size())

        if mask is not None:
            mask = mask.repeat(n_head, 1)
        output, attn = self.attention(qs, ks, vs, mask=mask) # mini*head, len_c, d_k
        '''
        1sent_vecs: torch.Size([10, 1024])
        q_size: torch.Size([10, 1024])
        k_size: torch.Size([10, 1024])
        qs_size: torch.Size([2, 5, 2, 512])
        vs_size: torch.Size([2, 5, 2, 512])
        ks_size: torch.Size([2, 5, 2, 512])
        qs2_size: torch.Size([4, 5, 512])
        vs2_size: torch.Size([4, 5, 512])
        ks2_size: torch.Size([4, 5, 512])
        out_put size: torch.Size([4, 5, 512])
        out_put size: torch.Size([2, 10, 512])
        out_put size: torch.Size([10, 1024])
        '''
...
class CustomizedEmbedding(nn.Module):
    def __init__(self, concept_num, concept_in_dim, concept_out_dim, use_contextualized=False,
        ...
        if concept_in_dim != concept_out_dim:
            self.cpt_transform = nn.Linear(concept_in_dim, concept_out_dim) # 1024->200
    ...
    def forward(self, index, contextualized_emb=None):
        ...
        if contextualized_emb is not None:
            ...
            if hasattr(self, 'cpt_transform'): # 判断是否有该属性
